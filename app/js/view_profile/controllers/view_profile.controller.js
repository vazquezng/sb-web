const ProfileController = /** @class */ (function () {
  function ProfileController(Canchas, LoginService, $http, $state, $scope, PATHS, Upload, $uibModal, toaster, $rootScope) {
    ...
    $scope.$watch('image', (newImage, lastImage) => {
    ...
    $scope.save = function () {
      vm.isSetted = false;
      for (let _i = 0, _a = vm.availability; _i < _a.length; _i++) {
        const entry = _a[_i];
  ...
  ProfileController.prototype.updateChecks = function (name, value) {
    ...
    if (name == 'always') {
      ...
      this.availability[this.tabIndex] = {
        allDay: value, morning: value, evening: value, night: value,
      };
  ...
  ProfileController.prototype.openModalAvailable = function () {
    const vm = this;
    const $paramObj = { user_id: vm.user.id };
    this.$http.post(`${this.PATHS.api}/user/retrieveUserAvailability`, $paramObj).then((resp) => {
  ...
  ProfileController.prototype.saveAvailability = function () {
    const params = { availability: this.availability };
    const vm = this;
    this.$http.post(`${this.PATHS.api}/user/saveAvailability`, params).then((resp) => {
  ...
  ProfileController.prototype.updateAvailability = function (weekDay, time) {
    const index = this.availabilityList.indexOf(`${weekDay}-${time}`);
    if (index == -1) {
  ...
  ProfileController.prototype.getCountryFromAddress = function () {
    for (let i = 0; i < this.address.address_components.length; i++) {
      for (let j = 0; j < this.address.address_components[i].types.length; j++) {
